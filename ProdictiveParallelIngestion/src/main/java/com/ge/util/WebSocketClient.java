...
import com.ge.api.IngestionResponse;
import com.ge.exception.ExSessionOpen;
...
public class WebSocketClient {
	...
	public boolean getWebsocketConnection() {
		try {
			...
			//settings.setServer("http://cis-india-pitc-bangalorez.proxy.corporate.ge.com:80");
			ws = factory.createSocket(ingestUrl);
			ws.addHeader("Predix-Zone-Id", zoneId);
			ws.addHeader(HttpHeaders.AUTHORIZATION,
					"Bearer " + restTemplate.getAccessToken());
	...
	public void connect(String server, String accessToken, String zoneId)
			throws DeploymentException, IOException {
		userSession = WebSocketUtils.openWebSocket(server, this, zoneId,
				accessToken);
		...
		if (session.getMessageHandlers().size() <= 0) {
			session.addMessageHandler(new MessageHandler.Whole<String>() {
				...
				public void onMessage(String message) {
					...
					IngestionResponse resp = new Gson().fromJson(message,
