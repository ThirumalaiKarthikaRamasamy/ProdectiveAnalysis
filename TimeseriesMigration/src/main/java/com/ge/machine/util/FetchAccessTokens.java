...
public class FetchAccessTokens {
	...
	public String getBasicEnvToken(){
		try {
			log.info("Fetching token for Basic Environment");
	...
	private String getToken(String clientId, String clientSecret, String uaaUrl){
		...
		try{
			...
			stopWatch.start();
			log.info("Setting up configuration for fetching access token");
			...
			if(response.getStatusCode().equals(HttpStatus.OK)){
				...
				stopWatch.stop();
				log.info("Token fetch completed in: "+stopWatch.getTotalTimeSeconds()+"s");
			}else{
				stopWatch.stop();
