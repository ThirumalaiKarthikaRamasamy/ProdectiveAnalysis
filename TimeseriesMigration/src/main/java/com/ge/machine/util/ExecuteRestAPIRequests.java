    ...
    public String call() throws IOException
	{
		...
		try{
			...
			if(response.has("tags")){
				try{
					response = response.getJSONArray("tags").getJSONObject(0);
					log.info("Recieved data from timseries: "+response.get("name"));
					if(response.has("results")) {
						tempArray = response.getJSONArray("results");
						if(tempArray.length()>0) {
							try {
								response.put("datapoints",tempArray.getJSONObject(0).getJSONArray("values"));
							}catch(Exception e) {
								log.error("Error retrieving data list from timeseries data.");
							}
							response.put("attributes",attributes);
							response.remove("results");
							response.remove("stats");
						}
					}
				}catch(Exception e){
					response = new JSONObject(responseReader.readLine());
				}
			}
